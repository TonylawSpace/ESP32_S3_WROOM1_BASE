...
#  @micropython.native
  def text0( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
...
#   @micropython.native
  def text(self, string, x, y, color):
    ...
    for char in string:
        ...
        if c < 32 or c > 126:
            c = 32  # 空格处理非打印字符
        index = (c - 32) * font.height
        for row in range(font.height):
            line = font.data[index + row]
            for col in range(font.max_width):
                if line & 0x80:
                    # 将 x + col 和 y + row 组合成一个元组
                    pos = (x + col, y + row)
